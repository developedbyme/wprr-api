<?php
	...
	class AbstractField {
		...
		public function format_value($value) {
			...
			$field_type = $this->_field_template->get_field_type();
			if($field_type) {
		...
		public function get_value() {
			...
			$storage_type = $this->_field_template->get_storage_type();
			if($storage_type) {
				switch($storage_type->get_slug()) {
					case "meta":
						$meta_field = $this->_field_template->get_post()->get_meta('dbmtc_meta_name');
						$return_value = $this->_post->get_meta($meta_field);
						break;
					case "relation-flag":
						$relation_path = $this->_field_template->get_post()->get_meta('dbmtc_relation_path');
						$return_value = $this->_post->has_term($wprr_data_api->wordpress()->get_taxonomy('dbm_relation')->get_term($relation_path));
						break;
					case "single-relation":
						$relation_path = $this->_field_template->get_post()->get_meta('dbmtc_relation_path');
						$term = $this->_post->get_single_term_in_with_descendants($wprr_data_api->wordpress()->get_taxonomy('dbm_relation')->get_term($relation_path));
						if($term) {
					...
					case "multiple-relation":
						$relation_path = $this->_field_template->get_post()->get_meta('dbmtc_relation_path');
						$terms = $this->_post->get_terms_in_with_descendants($wprr_data_api->wordpress()->get_taxonomy('dbm_relation')->get_term($relation_path));
