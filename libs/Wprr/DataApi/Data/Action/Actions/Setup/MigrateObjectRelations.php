<?php
	...
	// \Wprr\DataApi\Data\Action\Actions\Setup\MigrateObjectRelations
	class MigrateObjectRelations {
		...
		public static function apply_action($return_value, $data) {
			//var_dump("MigrateObjectRelations::apply_action");
			...
			*/
			...
			$type_parent_term = $wprr_data_api->wordpress()->get_taxonomy('dbm_type')->get_term('object-relation');
			$term_id = $type_parent_term->get_id();
			$limit = 1000;
			
			$sql = 'SELECT '.DB_TABLE_PREFIX.'posts.ID as id FROM '.DB_TABLE_PREFIX.'posts LEFT JOIN '.DB_TABLE_PREFIX.'dbm_object_relations ON '.DB_TABLE_PREFIX.'posts.ID = '.DB_TABLE_PREFIX.'dbm_object_relations.id INNER JOIN '.DB_TABLE_PREFIX.'term_relationships ON '.DB_TABLE_PREFIX.'posts.ID = '.DB_TABLE_PREFIX.'term_relationships.object_id WHERE '.DB_TABLE_PREFIX.'dbm_object_relations.id IS NULL AND post_status in ("publish","draft","private") AND post_type in ("dbm_object_relation") AND '.DB_TABLE_PREFIX.'term_relationships.term_taxonomy_id = '.$term_id.' LIMIT '.$limit;
			...
			foreach($ids as $id) {
				...
				$sql = 'INSERT INTO '.DB_TABLE_PREFIX.'dbm_object_relations (id, fromId, toId, type, startAt, endAt) SELECT '.$id.', '.$from.', '.$to.', id, '.$start_at.', '.$end_at.' FROM '.DB_TABLE_PREFIX.'dbm_object_relation_types WHERE path = "'.$type->get_slug().'";';
		...
		public static function test_import() {
			echo("Imported \Wprr\DataApi\MigrateObjectRelations<br />");
		}
	}
?>