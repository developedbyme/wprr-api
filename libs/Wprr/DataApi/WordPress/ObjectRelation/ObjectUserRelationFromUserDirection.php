<?php
	...
	// \Wprr\DataApi\WordPress\ObjectRelation\ObjectUserRelationFromUserDirection
	class ObjectUserRelationFromUserDirection {
		...
		public function get_types() {
			if(!$this->_has_all_types) {
				...
				if(defined("READ_OBJECT_RELATION_TABLES") && READ_OBJECT_RELATION_TABLES) {
					...
					$sql = "SELECT DISTINCT $types_table.path as path FROM $types_table INNER JOIN $relations_table ON $relations_table.type = $types_table.id WHERE $relations_table.userId = $user_id";
		...
		public function __toString() {
			return "[ObjectUserRelationFromUserDirection]";
		}
		
		public static function test_import() {
			echo("Imported \Wprr\DataApi\ObjectUserRelationFromUserDirection<br />");
		}
	}
?>